...
def criar():
    ...
    if request.method == 'POST':
        ...
        resultado = PlaylistController.criar_playlist(current_user.id, nome, descricao, publica)
        if resultado['success']:
...
def detalhes(playlist_id):
    ...
    resultado = PlaylistController.obter_playlist(playlist_id, usuario_id, tenant_id=tenant_id)

    if not resultado['success']:
        flash(resultado.get('message', 'Playlist nao encontrada'), 'error')
        return redirect(url_for('music.index'))

    playlist = resultado['playlist']
    is_owner = current_user.is_authenticated and playlist['usuario_id'] == current_user.id
    return render_template('playlist_detalhes.html', playlist=playlist, is_owner=is_owner)
...
def adicionar_musica(playlist_id, musica_id):
    ...
    resultado = PlaylistController.adicionar_musica(playlist_id, current_user.id, musica_id)
    if resultado['success']:
        flash(resultado['message'], 'success')
    else:
        flash(resultado['message'], 'error')
...
def publicas():
    """Lista playlists publicas."""
    tenant_id = current_user.tenant_id if current_user.is_authenticated else None
    resultado = PlaylistController.obter_playlists_publicas(limite=50, tenant_id=tenant_id)
